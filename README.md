...
function doSpam(element) {
  if(element.innerHTML == 'GASS'){
    var input = getElementByXpath("//*[@id=\"main\"]/footer/div[1]/div[2]/div/div[2]");
    if(input.innerHTML == '' || input.innerHTML == null){
      window.alert('Jan Banyak-Banyak, Ibo Wak-_-');
      return;
    }
    element.innerHTML = 'ALAH TU';
    message = input.innerHTML;
    var interval = parseInt (prompt('Jan Banyak-Banyak, Ibo Wak-_-:', '500'));
    repeatingSpamFunction = window.setInterval(function(){
      sendMessage();
    }, interval);
  } else {
    element.innerHTML = 'GASS';
